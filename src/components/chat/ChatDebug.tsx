...
export const ChatDebug: React.FC = () => {
  ...
  const { connectionState, isConnecting, error, connect, disconnect } =
    useChat();
    ...
    <div className="fixed bottom-4 right-4 bg-gray-800 text-white p-4 rounded-lg shadow-lg max-w-sm z-50">
      ...
      <div className="space-y-2 text-sm">
        <div>
          <strong>User:</strong>{" "}
          {user
            ? `${user.email || user.userName || "Unknown"} (${user.id})`
            : "Not logged in"}
        </div>
        ...
        {isConnecting && <div className="text-yellow-400">Connecting...</div>}
